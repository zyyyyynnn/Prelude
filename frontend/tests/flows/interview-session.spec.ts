...
const LEGACY_TEST_ACCOUNT_PREFERENCES_KEY =
  'prelude-interview-session-preferences:legacy%3A6dcf74c9'
...
test('migrates legacy session preferences and persists local hide behavior', async ({ page }) => {
  ...
  const migrated = await page.evaluate((preferenceKey) => ({
  ...
  }), LEGACY_TEST_ACCOUNT_PREFERENCES_KEY)
  ...
  await page.reload()
  await expect(page.getByRole('button', { name: '打开会话 Java 后端工程师' })).toHaveCount(0)
  const persisted = await page.evaluate((preferenceKey) =>
    JSON.parse(localStorage.getItem(preferenceKey) ?? '{}'),
    LEGACY_TEST_ACCOUNT_PREFERENCES_KEY,
  )
