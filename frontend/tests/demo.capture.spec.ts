...
test('@smoke @demo captures the deterministic React demo chain', async ({ page }, testInfo) => {
...
/** Diagnostic evidence for the demo chain: it lands in the Playwright report, not in
 *  the tracked screenshot assets, which are generated by the surfaces capture suite. */
...
async function expectActiveInterviewGeometry(page: Page) {
  ...
  const thread = await visibleBox(page.locator('[data-slot="message-thread"]'))
  const composer = await visibleBox(page.locator('[data-slot="workspace-composer"]'))
...
async function expectStructuredReportGeometry(page: Page) {
  const viewport = await visibleBox(page.locator('[data-slot="workspace-report"]'))
  const report = await visibleBox(page.locator('[data-slot="structured-report"]'))
...
async function expectAnalyticsGeometry(page: Page) {
  const panels = page.locator('[data-slot="chart-grid"] > [data-slot="panel"]')
...
async function expectSettingsGeometry(page: Page) {
  ...
  const sidebar = await visibleBox(page.locator('[data-slot="settings-sidebar"]'))
  const main = await visibleBox(page.locator('[data-slot="settings-main"]'))
