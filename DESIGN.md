...
- 状态藏在组件内部时，实验台仍然不复制标记：把呈现层抽成产品拥有的无状态组件（回答组合器的 `AnswerComposerSurface` 即如此），产品容器供真状态、实验台供冻结状态。产品里不存在的控件与文案（自造的语音状态滑块、「通知」按钮）不是样例，是第二个真相源；同理，重复的选项数据（主题三选项、推理档位标签）由 `features/settings` 一处导出，实验台与设置面板各自引用。
...
- 实验台不重复整页已经给出的东西：整页样例（报告）里含有的分件不再单独立一块面板；被检阅的条目必须是产品里那组条目（分段控件即「登录 / 注册」与「面试 / 报告」），不为演示另编第三套；契约只在面板描述里说一次，示例正文不复述组件自己的行为。
