...
- 会滚动且内容里可能没有任何可聚焦元素的容器，自己必须是焦点点（`tabIndex={0}`）：`Panel` 的 `fill` 主体与面试文字流 `TranscriptScroll` 都按这条实现。键盘用户滚不动的折叠以下内容等于看不见——组件把 `overflow-y-auto` 画出来时，也就承担了这条可达性，调用点不需要重复声明。
