...
1. **推荐：Local app runtime**（Docker 管中间件，本机跑 Spring Boot + Vite，支持 Vite HMR 即时热重载）
2. **可选：Full Docker runtime**（全量容器化，用于部署验证）
...
├── start-real.bat             # 推荐：真实版 Local App Runtime（含 Vite HMR）
├── start-demo.bat             # 推荐：Demo Twin Local App Runtime
├── start-real-docker.bat      # 可选：真实版 Full Docker Runtime（部署验证）
└── start-demo-docker.bat      # 可选：Demo Twin Full Docker Runtime
```
...
- **运行模式**：推荐使用 Local app runtime (start-real.bat / start-demo.bat) 以获得 Vite HMR 热重载支持。Full Docker runtime (-docker.bat) 保留用于部署验证。
