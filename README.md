...
- PDF 简历解析、阶段化模拟面试与结构化求职训练报告
- SSE 指数退避重连 + Resilience4j 熔断降级，提升 LLM 调用链路韧性
- 语音链路基础架构与 WebSocket 通道已搭建，真实低延迟表现仍需专项验证
- 用户级 BYOK：OpenAI Responses、OpenAI Chat Completions、Anthropic Messages 三类自定义协议，API Key AES-256-GCM 加密
...
仓库包含 GitHub Actions 工作流（`.github/workflows/ci.yml`），在 Windows runner 上执行后端编译/测试，以及前端架构、契约、构建、关键流和 UI 质量门禁。详见 [docs/frontend/architecture.md](docs/frontend/architecture.md)、[docs/quality/ui-quality-system.md](docs/quality/ui-quality-system.md) 与 [docs/quality/local-review-checklist.md](docs/quality/local-review-checklist.md)。
