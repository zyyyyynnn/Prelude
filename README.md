...
| MySQL   | 8.4  | local runtime 使用本机或本机可达服务 |
| Redis   | 7.0  | local runtime 使用本机或本机可达服务 |
| RabbitMQ | 3-management | local runtime 使用本机或本机可达服务 |

运行模式边界见 [docs/runtime-modes.md](docs/runtime-modes.md)：`.bat` / `.ps1` 是 local runtime 入口，Docker runtime 使用 `docker compose up -d ...`。
...
2. 编辑 `application-local.yml`，填写 MySQL 密码、JWT secret、AES secret；模型可通过用户级 BYOK 设置页配置
...
- `.bat` 启动脚本只检查本机端口，不会自动启动 Docker 或中间件；`.ps1` 启动脚本可能尝试启动本机 MySQL，但 Redis 与 RabbitMQ 仍需提前可达。local runtime 与 Docker runtime 边界见 [docs/runtime-modes.md](docs/runtime-modes.md)。
