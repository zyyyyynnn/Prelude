...
public abstract class AbstractChatCompletionsProvider implements LlmProvider {
    ...
    private String invoke(LlmInvocation invocation, boolean stream, Consumer<String> onDelta) {
        ...
        try {
            ...
            if (!invocation.attachments().isEmpty() && "deepseek".equals(providerKey)) {
                throw BusinessException.badRequest("当前 DeepSeek 模型不支持图片附件，请切换模型服务");
            }
            payload.put(
                "messages",
                invocation.attachments().isEmpty()
                    ? invocation.messages()
                    : LlmMultimodalMessages.openAiChat(invocation.messages(), invocation.attachments())
            );
    ...
    private String extractContent(String body) throws JacksonException {
