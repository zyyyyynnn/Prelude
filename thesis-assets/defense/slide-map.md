...
| 第 4 页 | 总体架构 | `thesis-assets/evidence/diagrams/fig-3.3-system-architecture.png` | 第三章 3.4 | 讲清前端、后端、MySQL、Redis、RabbitMQ、LLM Provider 和 dev fixture 的关系。 | RabbitMQ 只表示报告生成异步任务队列 |
...
| 第 10 页 | 测试验证 | `functional-cases-2026-06.md`、`quality-gates-2026-06-19.md`、`test-evidence-matrix-2026-06.md` | 第五章 | 展示 TC-01 到 TC-12、后端测试、前端 build、audit、CI 门禁包括 BYOK verify、dark verify、Sentrux、JaCoCo report、`verify:ui` UI 静态 guardrail、`verify:tokens` token schema、`verify:a11y` 仅 critical axe violations；`capture:visual` 作为 CI artifact-only 上传 17 个场景 PNG。 | 强调 JaCoCo report-only；`verify:ui` 是 UI 静态 guardrail 与 semantic sizing 红线扫描，不是全量视觉回归；`verify:a11y` 只 fail critical，serious 仍记入 backlog；`capture:visual` 不做像素 diff |
