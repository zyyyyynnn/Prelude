...
| 异步报告生成 | `/finish` 接口 200ms 内快速返回，报告生成任务通过 Redis 队列异步解耦，完成后 SSE 推送 `report_ready` 事件。 |
...
| RabbitMQ 是否已经实现？ | 当前已确认将 RabbitMQ 作为后续升级项，用于替换 Redis List 报告任务队列。但当前代码尚未补齐 AMQP 依赖、Docker Compose 服务、生产者/消费者和测试证据，所以不能写成已实现能力。现阶段可展示的是 Redis List 轻量级异步报告任务队列，RabbitMQ 属于后续补齐范围。 |
