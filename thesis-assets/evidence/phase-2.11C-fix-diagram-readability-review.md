...
### 14.2 比例与人工检查
- **截断原因排查与修复说明**：先前版本的 `%%{init: ...}%%` 配置块中使用了单引号（如 `'flowchart'`），导致 Mermaid CLI (Puppeteer) 静默忽略了 `fontFamily` 与 `nodePadding` 的渲染配置，从而在非默认渲染环境下引发了文字的截断问题。修复方案为将所有配置键值全部重构为符合 Mermaid 严格语法的双引号（`"`），并对所有多行节点的每一行 Label 内容进行 `&nbsp;` 实体空格填充缓冲以防末尾字符截断（此为多行节点逐行留白修复，彻底解决了图3.3中 "RabbitMQ" 尾部 "Q" 的截断问题，以及图3.2中所有表实体字段的文字截断风险），同时将 `'nodePadding': '15px'` 写入 `themeVariables`，并配置合理的 `nodeSpacing`/`rankSpacing`。
- **新版 viewBox 比例核验**：
  | 图 | viewBox | 宽高比 | 是否通过 | 说明 |
  | :--- | :--- | :---: | :---: | :--- |
