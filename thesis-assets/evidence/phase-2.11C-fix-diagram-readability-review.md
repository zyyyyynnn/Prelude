...
- **图3.1 标签精简**：优化了 `语音扩展` 规划节点描述，并将 nodeSpacing 精细下调至 95，使构图更紧凑。

### 14.2 比例与人工检查
- **截断原因排查与修复说明**：先前版本的 `%%{init: ...}%%` 配置块中使用了单引号（如 `'flowchart'`），导致 Mermaid CLI (Puppeteer) 静默忽略了 `fontFamily` 与 `nodePadding` 的渲染配置，从而在非默认渲染环境下引发了文字的截断问题。修复方案为将所有配置键值全部重构为符合 Mermaid 严格语法的双引号（`"`），并对所有节点的 Label 内容进行空格填充缓冲（例如 `user([" 求职者 (用户) "])`），同时将 `'nodePadding': '15px'` 写入 `themeVariables`，同时配置合理的 `nodeSpacing`/`rankSpacing`。
- **新版 viewBox 比例核验**：
  | 图 | viewBox | 宽高比 | 是否通过 | 说明 |
  | :--- | :--- | :---: | :---: | :--- |
