...
| 图3.3 | 图 | 系统整体架构图 | `thesis-assets/evidence/diagrams/fig-3.3-system-architecture.png` | 第三章 | `fig-3.3-system-architecture.mmd` | 已复核 / 待正文图号冻结 | 是 | 图 3.3 明确展示 MySQL、Redis、RabbitMQ 三类中间件职责。RabbitMQ 表示报告生成异步任务队列；当前不扩展为完整可靠消息处理方案。dev fixture 仅表示 local/dev 本地验收辅助能力，不进入 Full Docker / prod 默认路径。 |
