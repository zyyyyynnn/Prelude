...
| 图3.3 | 图 | 系统整体架构图 | `thesis-assets/evidence/diagrams/fig-3.3-system-architecture.png` | 第三章 | `fig-3.3-system-architecture.mmd`、`modular-monolith-boundary-hardening-2026-07-13.md` | 已刷新至 `851fa5b` / 待复核 | 待复核 | 图 3.3 展示模块化单体、应用端口与基础设施职责，不表示模块可独立部署。RabbitMQ 仅表示报告生成异步任务队列；dev fixture 仅用于 local/dev 本地验收。 |
...
| 补充证据 | 表 | 构建与自动化验证记录 | `thesis-assets/evidence/test-data/quality-gates-2026-07-13.md` | 第五章 | 两轮重构后的后端测试、前端构建、Sentrux、三个核心 application 包 70% instruction coverage 门禁、npm audit、BYOK/dark/UI/token/a11y 验证；`capture:visual` 为 artifact-only | 待复核 / 未作为当前正文编号表 | 待复核 | 70% 门禁不代表全仓覆盖率；Sentrux 不等同全量架构证明；`verify:ui` 与 `verify:tokens` 不等同全量视觉回归；`verify:a11y` 仅 critical-only；`capture:visual` 不等同像素 diff blocking gate |
