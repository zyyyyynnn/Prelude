...
规范优先级：
workflow-governance.md > AGENTS.md > thesis-assets/README.md
...
2. 提交版 DOCX/PDF 只能在 Word/WPS 人工终审后产生。
3. 证据未锁定，不允许调起 paper-spine-rewrite。
4. 外部研究未进入 literature/ 或 evidence/ 的研究缓冲区，不允许进入正文。
5. 图表未登记，不允许在正文中引用。
6. PaperSpine 只能单章执行。
7. 严禁全文一次性送入 PaperSpine。
8. Deep Research / NotebookLM 输出不得直接覆盖 chapters/*.md。
9. nature-figure 不得脱离真实代码、DDL、接口时序、测试数据自行想象图表。
10. 禁止使用“查重反制”“规避检测”等目标或表述。
...
| 阶段 5：Word/PDF 人工终审 | 引用已冻结 | 人工导出提交版 PDF 并锁定 | 发现排版乱码 |
...
1. submitted-thesis.docx：本地人工终审后的提交版 DOCX，不由 Agent 自动生成。
2. submitted-thesis.pdf：由人工终审 DOCX 导出的提交版 PDF。
3. 最终提交后，应记录 commit SHA 或 Git tag。
4. 任何提交版文件如需入库，必须由用户明确授权。
...
不得以 gzu-thesis-template.docx 作为正文修改源。
