...
| 质量门禁证据 | `thesis-assets/evidence/test-data/quality-gates-2026-06-19.md` | CI 门禁与本地预检质量门禁（含 `verify:ui` / `verify:tokens` / `verify:a11y` / `capture:visual`） | 可用 | JaCoCo report-only；`verify:ui` / `verify:tokens` / `verify:a11y` 为 CI blocking；`capture:visual` 为 CI artifact-only；详细限制见对应章节 |
...
- 当前基线为双字段：阶段 3 原始 freeze 审查基线 `4b2e967`；当前文档同步基线 `ffb617a`（`origin/main` HEAD）。阶段 3 准备工作只完成 evidence、图表登记和答辩材料口径核对。
...
- 质量门禁：CI 包含 whitespace diff check（PR 用 merge-base 取得 diff 起点）、Sentrux、后端测试、JaCoCo report artifact、npm audit、前端 build、`verify:ui`、`verify:tokens`、`verify:byok`、`verify:dark`、`verify:a11y`；`capture:visual` 作为 artifact-only（`continue-on-error: true`）。
